...
class HarfexEngine:
    ...
    def __init__(self):
        ...
        self.plexiglas_offset = 0.0; self.foam_offset = 0.0
    ...
    def build(self, fast=False):
        ...
        try:
            ...
            if self.wall_type == 0:
                v, f = MB.extrude_geom(wg, z_wall_start, h,
                                       cap_bottom=cap_bot, cap_top=True)
            else:
    ...
    def export_cover_stl_bytes(self) -> bytes:
        ...
        fa = fa[:,[0,2,1]]
        return self._arrays_to_stl_bytes(va, fa)

    # ─────────────────────────────────────────────────────────────────────────
    # 3MF export
    # ─────────────────────────────────────────────────────────────────────────
    def export_3mf_bytes(self, include_face=True, include_cover=False) -> bytes:
        """Body (+ opsiyonel yüz + kapak) → 3MF bytes."""
        import zipfile
        ...
        if include_cover:
            try:
                ...
                # parse the STL bytes back to v/f for 3MF embedding
                # (simplest: skip cover in 3MF for now — use separate STL)
            except Exception:
                pass
