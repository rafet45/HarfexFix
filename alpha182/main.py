...
from PySide6.QtCore import Signal, QPoint, QPointF, QRectF, Qt, QTimer
from PySide6.QtGui import (
    QColor, QFont, QIcon, QPainter, QPainterPath, QPixmap, QPolygonF,
...
from PySide6.QtWidgets import (
    ...
    QMenu, QMenuBar, QMessageBox, QPushButton, QRadioButton, QScrollArea,
    QScrollBar, QStackedWidget, QStatusBar, QVBoxLayout, QWidget,
)
try:
...
class ModelSlot:
    ...
    _ATTRS = [
        ...
        '_wall_gl_item', '_face_gl_item', '_band_gl_item',
        '_last_disp_v', '_last_disp_f',
        'color_wall', 'color_face', 'color_cover', 'color_band',
        'band_on', 'band_pos', 'band_h',
    ]
    ...
    def __init__(self, label="Model", x_offset=0.0, filename=""):
        ...
        self.color_cover = (0.65, 0.65, 0.65, 1.0)
        self.color_band = (0.133, 0.333, 0.80, 1.0)  # strip / kuşak default (#2255cc)
        self.band_on  = False
        self.band_pos = 0.0
        self.band_h   = 0.0
        self._band_gl_item = None
        self._last_disp_v = None; self._last_disp_f = None
...
class Viewport(QWidget):
    def __init__(self):
        ...
        self.color_cover = (0.65, 0.65, 0.65, 1.0)   # back cover default
        self.color_band  = (0.133, 0.333, 0.80, 1.0) # strip / kuşak default (#2255cc)
        self.band_on  = False
        self.band_pos = 0.0
        self.band_h   = 0.0
    ...
    def set_display_mode(self, mode: str):
        ...
        _band_items = [self._band_gl_item] if self._band_gl_item is not None else []
        for item in self.mesh_items + self.cover_items + _band_items:
    ...
    def show_color_dialog(self, parent=None):
        """Open color assignment dialog. Mirrors the web tool's m-color modal:
        left column = 4 filament color swatches (Gövde/Strip/Yüz/Kapak),
        right column = band on/off toggle bar + vertical slider + mockup preview + height input."""
        ...
        def _swatch_style(qc):
            return (f"background:{qc.name()}; border:2px solid #555; border-radius:5px;")

        vp = self

        # ── Mockup canvas (mirrors web #mock-canvas) ────────────────────────
        class _Mockup(QWidget):
            def __init__(self):
                super().__init__()
            ...
            def paintEvent(self, ev):
                ...
                p.fillRect(pad, 0, W - pad*2, H, _rgba_to_qcolor(vp.color_wall))
                p.fillRect(pad+wt, 0, W - pad*2 - wt*2, H, QColor(0, 0, 0, 46))
                if vp.band_on and vp.band_h > 0:
                    h = max(vp.height_mm, 0.001)
                    bp, bh = vp.band_pos, vp.band_h
                    y_top = H - (bp + bh) / h * H
                    y_bot = H - bp / h * H
                    yT = max(0.0, min(H, y_top)); yB = max(0.0, min(H, y_bot))
                    p.fillRect(pad, int(yT), W - pad*2, int(yB - yT), _rgba_to_qcolor(vp.color_band))
                    p.setPen(QColor(255, 255, 255, 77))
                    p.drawRect(pad, int(yT), W - pad*2 - 1, int(yB - yT) - 1)
                p.setPen(QColor('#666'))
                p.drawRect(pad, 0, W - pad*2 - 1, H - 1)
                p.end()
        ...
        dlg.setStyleSheet(
            ...
            "QLabel#sectionLbl{font-size:10px; font-weight:bold; color:#888; "
        ...
        _TOGGLE_ON  = "background:#3a9e44;color:#fff;font-size:9px;letter-spacing:.5px;border:none;border-radius:4px 4px 0 0;"
        _TOGGLE_OFF = "background:#444;color:#fff;font-size:9px;letter-spacing:.5px;border:none;border-radius:4px 4px 0 0;"

        root = QVBoxLayout(dlg); root.setSpacing(14); root.setContentsMargins(18, 16, 18, 16)
        ...
        def make_swatch(label_text, rgba, apply_fn):
            box = QVBoxLayout(); box.setSpacing(4)
            lbl = QLabel(label_text); lbl.setObjectName("sectionLbl")
            box.addWidget(lbl)
            qc_ref = [_rgba_to_qcolor(rgba)]
            btn = QPushButton(); btn.setFixedSize(160, 36)
            ...
            def pick():
                ...
                if chosen.isValid():
                    ...
                    apply_fn(_qcolor_to_rgba(chosen))
        ...
        def apply_band(rgba):
            self.color_band = rgba
            if self._band_gl_item is not None:
                self._band_gl_item.setColor(rgba)
            mockup.update()
        ...
        def apply_cover(rgba):
            self.color_cover = rgba
            for item in self.cover_items:
                item.setColor(rgba)

        make_swatch("Gövde / Channel Body  (F1)", self.color_wall, apply_wall)
        make_swatch("Kuşak / Strip", self.color_band, apply_band)
        make_swatch("Ön Yüz / Front Face  (F2)", self.color_face, apply_face)
        make_swatch("Arka Kapak / Back Cover  (F3)", self.color_cover, apply_cover)

        # ── Sağ: toggle bar + slider + mockup + yükseklik ───────────────────
        right = QVBoxLayout(); right.setSpacing(0)
        right.setAlignment(_Qt.AlignHCenter)
        cols.addLayout(right)

        toggle_btn = QPushButton("ON" if self.band_on else "OFF")
        toggle_btn.setStyleSheet(_TOGGLE_ON if self.band_on else _TOGGLE_OFF)
        toggle_btn.setFixedSize(104, 14)
        right.addWidget(toggle_btn)

        mock_row = QHBoxLayout(); mock_row.setSpacing(6)
        right.addLayout(mock_row)

        slider = QSlider(_Qt.Vertical)
        slider.setRange(0, 100)
        h0 = self.height_mm if self.height_mm > 0 else 45.0
        slider.setValue(int(round((self.band_pos / h0) * 100)) if h0 > 0 else 50)
        slider.setFixedHeight(220)
        mock_row.addWidget(slider)

        mockup = _Mockup()
        mock_row.addWidget(mockup)

        h_row = QHBoxLayout(); h_row.setSpacing(6)
        h_row.setAlignment(_Qt.AlignHCenter)
        h_spin = QDoubleSpinBox(); h_spin.setRange(0.0, 1000.0)
        h_spin.setDecimals(1); h_spin.setValue(self.band_h); h_spin.setFixedWidth(70)
        h_unit = QLabel("mm"); h_unit.setStyleSheet("color:#777; font-size:10px;")
        h_row.addWidget(h_spin); h_row.addWidget(h_unit)
        right.addSpacing(6)
        right.addLayout(h_row)

        def on_toggle():
            self.band_on = not self.band_on
            toggle_btn.setText("ON" if self.band_on else "OFF")
            toggle_btn.setStyleSheet(_TOGGLE_ON if self.band_on else _TOGGLE_OFF)
            self._update_band_mesh()
            mockup.update()

        def on_slider(val):
            h = self.height_mm if self.height_mm > 0 else 45.0
            self.band_pos = round(h * val / 100.0, 1)
            self._update_band_mesh()
            mockup.update()
        ...
        h_spin.valueChanged.connect(on_height)
    ...
    def clear_model(self):
        ...
        if self._band_gl_item is not None:
            try: self.view.removeItem(self._band_gl_item)
    ...
    def apply_mesh(self, verts, faces, boundary=None):
        ...
        # Keep displayed wall geometry for band/strip overlay rebuilds
        self._last_disp_v = _hv; self._last_disp_f = _hf
    ...
    # ── Band / Strip overlay (visual filament-color separation) ────────────────
    @staticmethod
    def _clip_poly_z(poly, z_val, keep_above):
        """Sutherland-Hodgman: clip a polygon (list of [x,y,z]) against a Z plane."""
        out = []
        n = len(poly)
        for i in range(n):
            a = poly[i]; b = poly[(i + 1) % n]
            a_in = (a[2] >= z_val) if keep_above else (a[2] <= z_val)
            b_in = (b[2] >= z_val) if keep_above else (b[2] <= z_val)
            if a_in:
                out.append(a)
            if a_in != b_in:
                t = (z_val - a[2]) / (b[2] - a[2])
                out.append([a[0] + t * (b[0] - a[0]), a[1] + t * (b[1] - a[1]), z_val])
        return out

    def _update_band_mesh(self):
        """Rebuild the band/strip color overlay by clipping the wall mesh between
        band_pos and band_pos + band_h on the Z axis (visual only, no manifold op)."""
        if self._band_gl_item is not None:
            try:
                self.view.removeItem(self._band_gl_item)
            except Exception:
                pass
            self._band_gl_item = None
        if not self.band_on or not (self.band_h > 0):
            return
        v, f = self._last_disp_v, self._last_disp_f
        if v is None or f is None:
            return
        bp     = float(self.band_pos)
        bp_top = bp + float(self.band_h)
        eps    = 0.05   # small outward push to avoid z-fighting with the wall
        verts  = []
        for tri in f:
            i0, i1, i2 = int(tri[0]), int(tri[1]), int(tri[2])
            p0, p1, p2 = v[i0], v[i1], v[i2]
            zmn = min(p0[2], p1[2], p2[2]); zmx = max(p0[2], p1[2], p2[2])
            if zmx < bp or zmn > bp_top:
                continue
            poly = [[float(p0[0]), float(p0[1]), float(p0[2])],
                    [float(p1[0]), float(p1[1]), float(p1[2])],
                    [float(p2[0]), float(p2[1]), float(p2[2])]]
            poly = self._clip_poly_z(poly, bp, True)
            if len(poly) < 3:
                continue
            poly = self._clip_poly_z(poly, bp_top, False)
            if len(poly) < 3:
                continue
            a = np.asarray(poly[0]); b = np.asarray(poly[1]); c = np.asarray(poly[2])
            nrm = np.cross(b - a, c - a)
            nlen = np.linalg.norm(nrm)
            nrm = (nrm / nlen * eps) if nlen > 1e-9 else np.zeros(3)
            for k in range(1, len(poly) - 1):
                for pt in (poly[0], poly[k], poly[k + 1]):
                    verts.append([pt[0] + nrm[0], pt[1] + nrm[1], pt[2] + nrm[2]])
        if not verts:
            return
        va = np.array(verts, dtype=np.float32)
        fa = np.arange(len(va), dtype=np.int32).reshape(-1, 3)
        _df, _de, _sh = self._disp_params()
        gl_mesh = gl.MeshData(vertexes=va, faces=fa)
        item = gl.GLMeshItem(meshdata=gl_mesh, smooth=False, drawFaces=_df, drawEdges=_de,
                             color=self.color_band, edgeColor=(0, 0, 0, 1),
                             shader=_sh, glOptions="opaque")
        self.view.addItem(item)
        self._band_gl_item = item
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        fm.addSeparator()
        fm.addAction(_t("menu_clear"),             self._new_page)
        fm.addSeparator()
    ...
    def _pick_cover_color(self):
        r, g, b, a = self.vp.color_cover
        start = QColor(int(r*255), int(g*255), int(b*255))
        c = QColorDialog.getColor(start, self, "Back Cover Rengi")
        if not c.isValid(): return
        col = (c.redF(), c.greenF(), c.blueF(), 1.0)
        self.vp.color_cover = col
        for item in self.vp.cover_items:
            item.setColor(col)

    # ── Sahneyi temizle ───────────────────────────────────────────────────────
    def _new_page(self):
        """'Yeni Sayfa' — discard all current work and reset to a fresh empty page."""
        ...
        if self.vp._band_gl_item is not None:
            try: self.vp.view.removeItem(self.vp._band_gl_item)
            except Exception: pass
        ...
        self.vp.last_face_v = self.vp.last_face_f = None
        self.vp._wall_gl_item = self.vp._face_gl_item = None
        self.vp._band_gl_item = None
        self.vp._last_disp_v = self.vp._last_disp_f = None
        self.vp.selected_obj_key  = None
        self.vp.selected_obj_keys = set()
        self.vp.groups = {}
        # Reset colors / band-strip state to defaults
        self.vp.color_wall  = s0.color_wall
        self.vp.color_face  = s0.color_face
        self.vp.color_cover = s0.color_cover
        self.vp.color_band  = s0.color_band
        self.vp.band_on  = s0.band_on
        self.vp.band_pos = s0.band_pos
        self.vp.band_h   = s0.band_h
        # Reset wall/cover/face/tab parameters to defaults
        self.vp.wall_mm = s0.wall_mm; self.vp.height_mm = s0.height_mm
        self.vp.wall_type = s0.wall_type; self.vp.wall_taper = s0.wall_taper
        self.vp.face_mode = s0.face_mode; self.vp.face_thickness = s0.face_thickness
        self.vp.face_fill = s0.face_fill
        self.vp.face_fill_cell = s0.face_fill_cell
        self.vp.face_fill_wall = s0.face_fill_wall
        self.vp.face_fill_solid_pos = s0.face_fill_solid_pos
        self.vp.face_fill_border = s0.face_fill_border
        self.vp.mx = s0.mx; self.vp.my = s0.my
        self.vp.top_tab = s0.top_tab; self.vp.bot_tab = s0.bot_tab
        self.vp.top_proj = s0.top_proj; self.vp.bot_proj = s0.bot_proj
        self.vp.top_tab_z = s0.top_tab_z; self.vp.bot_tab_z = s0.bot_tab_z
        self.vp.cover_ct = s0.cover_ct; self.vp.cover_wh = s0.cover_wh
        self.vp.cover_clearance = s0.cover_clearance; self.vp.cover_wt = s0.cover_wt
        self.vp.plexiglas_offset = s0.plexiglas_offset
        self.vp.foam_offset = s0.foam_offset
        # Reset left-panel input fields to defaults
        self.lp.height.setText("50")
        self.lp.wall.setText("3")
        self.lp.fp_val.setText("2.4")
        self.lp.arc.setText("8")
        self.lp.rc.setText("1.0")
        self.lp.simp.setText("0.05")
        # Clear undo/redo history — previous operations no longer apply
        self._undo_stack = []
